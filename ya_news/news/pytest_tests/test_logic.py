...
def test_anonymous_user_cant_create_comment(client,
    ...
    expected_url = (f'{login_url_reverse}?'
                    f'next={delete_url_reverse}')
    assertRedirects(response, expected_url)
    assert Comment.objects.count() == 0


def test_user_can_create_note(author_client, news,
                              author,
                              detail_url_reverse):
...
def test_author_can_delete_comment(author_client, detail_url_reverse,
                                   delete_url_reverse):
    comments_before_delete = Comment.objects.count()
    response = author_client.post(delete_url_reverse)
    assertRedirects(response,
                    f'{detail_url_reverse}#comments')
    assert Comment.objects.count() == comments_before_delete - 1
...
def test_user_cant_delete_comment_of_another_user(not_author_client,
    ...
    assert response.status_code == HTTPStatus.NOT_FOUND
    assert Comment.objects.count() == comments_before_delete
